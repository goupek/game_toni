...
class TTSWorker(threading.Thread):
    ...
    def _synthesize_and_save(self, text: str):
        ...
        if hasattr(self._voice, "synthesize_stream_raw"):
            print("[TTS] Using PiperVoice.synthesize_stream_raw")
        ...
        if hasattr(self._voice, "synthesize_ids_to_raw"):
            print("[TTS] Using PiperVoice.synthesize_ids_to_raw")
            phoneme_ids = self._voice.phonemize(text)
            audio_bytes = b"".join(self._voice.synthesize_ids_to_raw(phoneme_ids))
            self._save_pcm_bytes(audio_bytes, sr, text)
            return

        # --- synthesize() writes to a WAV writer/file ---
        if hasattr(self._voice, "synthesize"):
            print("[TTS] Using PiperVoice.synthesize")

            # First try a real temporary WAV file. This is often more robust than
            # an in-memory BytesIO object for libraries that expect a normal file.
            tmp_path = None
            try:
                ...
                print("[TTS] synthesize() to temp WAV failed. Full traceback:")
                traceback.print_exc()

                # Fall back to an in-memory WAV buffer, but initialize the header
                # explicitly so wave.close() does not fail with '# channels not specified'.
                try:
                    buf = io.BytesIO()
                    ...
                    print("[TTS] synthesize() to in-memory WAV also failed. Full traceback:")
                    traceback.print_exc()
                    raise RuntimeError(
                        f"Piper synthesize failed. temp_wav_error={file_exc!r}; memory_wav_error={mem_exc!r}"
                    ) from mem_exc
...
class VoiceAssistant:
    ...
    def run_forever(self):
        print("==============================================")
        print("Assistant running (VAD + wake + LLM stream + saving TTS to WAV files).")
