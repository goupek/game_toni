...
def ollama_chat_once(messages: List[Dict]) -> str:
    """Fetches the entire LLM response at once."""
    payload = {
        "model": OLLAMA_MODEL,
        "messages": messages,
...
# =========================
# TTS WORKER (WHOLE AUDIO SYNTHESIS)
# =========================
class TTSWorker(threading.Thread):
    ...
    def _synthesize_and_save(self, text: str):
        ...
        try:
            ...
            # Using standard file open instead of wave module to prevent header corruption
            with open(out_path, "wb") as wav_file:
                # Piper native synthesize method writes its own headers and whole audio stream directly
                self._voice.synthesize(text, wav_file)
                
            print(f"[TTS] Successfully saved whole audio to {out_path}")
