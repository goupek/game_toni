...
# Whisper config from your second file
...
class WhisperSTT:
    def __init__(self):
        print(f"[Init] Loading Whisper model: size={WHISPER_MODEL_SIZE}, device={WHISPER_DEVICE}, compute={WHISPER_COMPUTE}")
    ...
    def transcribe_bytes(self, audio_bytes: bytes, sample_rate: int) -> str:
        try:
            ...
            audio = resample_audio_float32(audio, sample_rate, WHISPER_SR)
...
class VadAudioCapture:
    ...
    def step(self, frame: bytes, max_utt_sec: float) -> CaptureOut:
        ...
        if ended:
            ...
            self.reset()
            return CaptureOut(final=audio_bytes)
...
class VoiceAssistant:
    ...
    def _transcribe_utterance(self, audio_bytes: bytes) -> str:
        text = self.whisper.transcribe_bytes(audio_bytes, self.sample_rate)
    ...
    def run_forever(self):
        ...
        with sd.RawInputStream(
            ...
            print("[Mic] Input stream open. Listening...")
            try:
                while True:
                    ...
                    if self.state.mode == "SLEEP":
                        out = self.capture.step(frame=frame, max_utt_sec=MAX_UTT_SLEEP_SEC)
                        if out.final:
                            text = self._transcribe_utterance(out.final)
                        ...
                        out = self.capture.step(frame=frame, max_utt_sec=MAX_UTT_ACTIVE_SEC)
                        if out.final:
                            user_text = self._transcribe_utterance(out.final).strip()
                            if user_text:
