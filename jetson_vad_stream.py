...
def llm_stream_to_tts_chunks(stream_iter: Iterable[str],
    ...
    Still chunked (not true audio packet streaming), but low perceived latency.
    """
    buf = ""
    spoken = 0
...
class AudioStreamer:
    ...
    This avoids gaps between TTS chunks.
    """
    def __init__(self):
...
class TTSWorker(threading.Thread):
    ...
    def say(self, text: str):
        ...
        if not text:
            return
        self.q.put(text)  # block if needed
    ...
    def _lazy_init(self):
        ...
        # Jetson Nano CPU: keep threads modest
        try:
            torch.set_num_threads(2)
        except Exception:
            pass

        print("[TTS] Loading Qwen3-TTS... first run may download weights.")
...
class VadVoskCapture:
    """
    Uses WebRTC VAD to detect speech segments and feeds them into Vosk.
    Maintains a pre-roll buffer so the start isn't clipped.
    """
...
class VoiceAssistant:
    def __init__(self):
        ...
        # Audio input queue of raw frames
        self.audio_q: "queue.Queue[bytes]" = queue.Queue(maxsize=400)

        # Output audio streamer + TTS worker
        ...
        # mic gate tail (avoid picking up last milliseconds of speaker output)
    ...
    def _make_recognizers(self):
        # Grammar-limited wake recognizer (reduces false triggers)
    ...
    def _audio_callback(self, indata, frames, time_info, status):
        ...
        # If assistant is speaking (buffer has audio), drop mic frames
        # Also keep a short tail gate after buffer drains.
    ...
    def _handle_user_text(self, text: str):
        ...
        # Stream LLM deltas and feed micro-chunks to TTS
        assistant_full = ""
        try:
            ...
            # We also want to store the final assistant message in history.
            # Easiest: call again non-streaming is expensive; instead, accumulate during streaming.
            # So we re-run streaming but that duplicates computation. Instead:
            # We reconstruct approx by joining spoken chunks; good enough for short answers.
            # (If you want exact text, we can store deltas while streaming and clamp.)
            # We'll store the concatenation of what we queued:
            # NOTE: TTS queue already has chunks; we rebuild with the same logic by collecting deltas once.
            # => We'll do it properly by collecting deltas in one pass:
        ...
        # Properly store assistant text: re-run stream but just for history is wasteful.
        # Better: do a single pass collecting deltas while chunking.
        # We implement that below by doing one pass with collection.

    def _respond_with_llm(self, user_text: str):
        """
        Do one streaming pass:
          - collect deltas to build assistant_full
          - yield micro-chunks to TTS
        """
        ...
        try:
            ...
            # We'll process deltas ourselves so we can both collect and chunk
            buf = ""
            spoken = 0
        ...
        # gate mic slightly after we finish queueing audio (prevents immediate feedback)
    ...
    def run_forever(self):
        ...
        with sd.RawInputStream(
            ...
            blocksize=self.frame_samples,  # exactly 1 VAD frame
            dtype="int16",
            channels=1,
                        ...
                        if out.final:
                            ...
                            if user_text:
                                ...
                                if contains_phrase(user_text, SLEEP_PHRASES):
                                    ...
                                    # refresh recognizer for next utterance
                                    self.rec_full = KaldiRecognizer(self.vosk_model, self.sample_rate)
